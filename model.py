...
try:
    ...
    class SubMConv3d_Mock(nn.Module):
        def __init__(self, in_channels, out_channels, *args, **kwargs):
            super().__init__()
            self.net = nn.Linear(in_channels, out_channels, bias=kwargs.get('bias', True))
        
        def forward(self, input):
            if hasattr(input, "features"):
                input.features = self.net(input.features)
                return input
            elif hasattr(input, "feat"):
                input.feat = self.net(input.feat)
                return input
            return self.net(input)

    # 2. Mock the Sparse Tensor (This fixes the replace_feature crash!)
    class SparseConvTensor_Mock:
        def __init__(self, features, indices=None, spatial_shape=None, batch_size=None, **kwargs):
            self.features = features
            # Dummy indices to prevent shape checking crashes
            self.indices = indices if indices is not None else torch.zeros(0, 4)
            self.spatial_shape = spatial_shape
            self.batch_size = batch_size

        def replace_feature(self, feature):
            # Returns a NEW mock tensor with the updated features, keeping the methods intact
            return SparseConvTensor_Mock(
                features=feature, 
    ...
    # 3. Bind them to the spconv namespace
    spconv = SimpleNamespace(
        SubMConv3d=SubMConv3d_Mock,
        SparseConvTensor=SparseConvTensor_Mock,
        modules=SimpleNamespace(is_spconv_module=lambda x: isinstance(x, SubMConv3d_Mock))
    )
    ...
    def sparsify(self, pad=96):
        ...
        try:
            sparse_conv_feat = spconv.SparseConvTensor(
                ...
                batch_size=int(self.batch[-1]) + 1,
            )
        except TypeError:
            # Fallback for our Mock object on Mac
            from addict import Dict
            sparse_conv_feat = Dict(features=self.feat)
            sparse_conv_feat.replace_feature = lambda x: Dict(features=x)
...
if __name__ == "__main__":
    ...
    # Let's also print what else the model returned in the Point dictionary
    print("Keys in output Point object:", list(output.keys()))